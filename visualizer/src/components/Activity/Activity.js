...
const Activity = ({
  ...
  const cdrIds = []
    .concat(
      ...cdr.filter((item) => item.from === data.id).map((item) => item.calls),
    )
    .map((item) => detailedCdr.find((obj) => obj.id === item));

  const ipdrIds = []
    .concat(
      ...ipdr.filter((item) => item.from === data.id).map((item) => item.calls),
    )
    .map((item) => detailedIpdr.find((obj) => obj.id === item));

  const finalValues = [].concat(cdrIds, ipdrIds).sort((a, b) => {
